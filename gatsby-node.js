  ...
  Promise.all(
    categories.map(({ category }) =>
      ...
      gatsbyUtilities.actions.createPage({
        ...
        // use the category template as the page component
        ...
        context: {
          ...
          // so our blog category template knows which blog category
...
/**
 * This function queries Gatsby's GraphQL server and asks for
 * All WordPress blog categories. If there are any GraphQL error it throws an error
 * Otherwise it will return the categories 🙌
 */
async function getCategories({ graphql, reporter }) {
  const graphqlResult = await graphql(/* GraphQL */ `
    query WpCategories {
      # Query all WordPress blog categories
      allWpCategory {
        edges {
          category: node {
            id
